...
// Also runs automatically on the cron schedule (see src/jobs/refresh.js).
adminRouter.post('/sync/:gw', async (req, res) => {
...
adminRouter.post('/sync/current', async (_req, res) => {
  const bootstrap = await fetchBootstrap({ forceRefresh: true });
  const gw = currentEventId(bootstrap);
  if (!gw) return res.status(400).json({ error: 'Could not determine current gameweek' });
...
adminRouter.post('/awards/quarterly/:quarter', async (req, res) => {
  const result = await recomputeQuarterlyAwards(Number(req.params.quarter));
  res.json(result);
});

adminRouter.post('/h2h/generate', async (req, res) => {
